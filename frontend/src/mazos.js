...
async function cargarGaleriaMazos() {
  ...
  contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Cargando mazos de la comunidad...</p>';
  try {
    await actualizarSetFavoritosUsuario();
    ...
    contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Error de conexión al cargar la galería de mazos.</p>';
  }
}
...
function renderizarGaleriaMazos(mazos) {
  ...
  if (!mazos || mazos.length === 0) {
    contenedor.innerHTML = '<p class="mensaje-mazo-vacio">No hay mazos públicos disponibles aún. ¡Sé el primero en crear uno!</p>';
    return;
  }
  ...
  mazos.forEach(mazo => {
    ...
    card.classList.add('tarjeta-mazo');
    ...
    const ratingBadgeHTML = mazo.promedio_puntuacion
      ? '<span class="badge-rating-galeria">⭐ ' + Number(mazo.promedio_puntuacion).toFixed(1) + ' <small>(' + (mazo.total_valoraciones || 1) + ')</small></span>'
      : '<span class="badge-rating-galeria" style="opacity: 0.6; border-color: #333;">⭐ Nuevo</span>';

    card.innerHTML =
      '<div class="tarjeta-mazo-header">' +
        '<div class="tarjeta-mazo-header-top">' +
          '<h3 class="tarjeta-mazo-titulo">' + nombreMazo + '</h3>' +
        '</div>' +
        '<div class="tarjeta-mazo-meta">' +
          '<span class="mazo-autor">Por: <strong>' + nombreUsuario + '</strong>' + (esPropio ? ' <span class="badge-tu-mazo">(Tú)</span>' : '') + '</span>' +
          '<div class="tarjeta-mazo-meta-badges">' +
            ratingBadgeHTML +
          '</div>' +
        '</div>' +
      '</div>' +
      '<div class="tarjeta-mazo-body">' +
        '<p class="tarjeta-mazo-desc">' + descripcion + '</p>' +
        '<div class="tarjeta-mazo-footer">' +
          '<span class="mazo-fecha">📅 ' + fechaCreacion + '</span>' +
...
async function cargarMisMazosSeccion(idUsuario) {
  ...
  if (!idUsuario) {
    contenedor.innerHTML = `
      <div class="banner-mazo-login">
        <div class="icono-mazo-login">🔒</div>
  ...
  contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Cargando tus mazos creados...</p>';
  ...
  try {
    ...
    if (mazos.length === 0) {
      contenedor.innerHTML = `
        <div class="banner-mazo-vacio">
          <div class="icono-mazo-vacio">🗂️</div>
    ...
    mazos.forEach(mazo => {
      ...
      card.innerHTML = `
        <div class="tarjeta-mazo-header">
          <div class="tarjeta-mazo-header-top">
            <h3 class="tarjeta-mazo-titulo">${sanitizarHTML(mazo.nombre_mazo)}</h3>
        ...
        <div class="tarjeta-mazo-body">
          <p class="tarjeta-mazo-desc">${desc}</p>
          <div class="tarjeta-mazo-footer">
            <button class="btn-primary btn-ver-mi-mazo" data-id="${mazo.id_mazo}">Ver Cartas</button>
            <button class="btn-secondary btn-editar-mi-mazo-seccion" data-id="${mazo.id_mazo}">✏️ Editar</button>
            <button class="btn-danger-mini btn-eliminar-mi-mazo-seccion" data-id="${mazo.id_mazo}" data-nombre="${sanitizarHTML(mazo.nombre_mazo)}">Eliminar</button>
          </div>
        </div>
      `;
      ...
      const btnEditar = card.querySelector('.btn-editar-mi-mazo-seccion');
      if (btnEditar) {
        btnEditar.onclick = (e) => {
          e.stopPropagation();
          if (typeof cargarMazoEnConstructorParaEditar === 'function') {
            cargarMazoEnConstructorParaEditar(mazo.id_mazo);
          }
        };
      }
    ...
    contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Error de conexión al cargar tus mazos.</p>';
  }
}
...
async function cargarMazosFavoritosSeccion(idUsuario) {
  ...
  contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Cargando tus mazos favoritos...</p>';
  ...
  try {
    ...
    if (favoritos.length === 0) {
      contenedor.innerHTML = `
        <div class="banner-mazo-vacio">
          <div class="icono-mazo-vacio">⭐</div>
    ...
    contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Error de conexión al cargar tus favoritos.</p>';
  }
}
...
function configurarFiltrosDeckbuilder() {
  const inputNombre = document.getElementById('filtro-nombre-constructor');
  const selectTipo = document.getElementById('filtro-tipo-constructor');
  const selectRaza = document.getElementById('filtro-raza-constructor');
  const selectCoste = document.getElementById('filtro-coste-constructor');
  const selectFuerza = document.getElementById('filtro-fuerza-constructor');
  const btnLimpiar = document.getElementById('btn-limpiar-filtros-constructor');
...
function renderizarCatalogoDeckbuilder(cartas) {
  const contenedor = document.getElementById('catalogo-lista-constructor');
  if (!contenedor) return;
  contenedor.innerHTML = '';
  if (!cartas || cartas.length === 0) {
    contenedor.innerHTML = '<p class="mensaje-mazo-vacio">No se encontraron cartas.</p>';
    return;
  }
  ...
  cartas.forEach(carta => {
    ...
    card.innerHTML = `
      ...
      <div class="carta-mazo-mini-info">
...
function actualizarVistaMazo() {
  ...
  if (mazoActual.length === 0) {
    contenedor.innerHTML = '<p class="mensaje-mazo-vacio">Tu mazo está vacío. Haz clic en las cartas del catálogo para agregarlas.</p>';
  }
  ...
  if (btnGuardar) {
    btnGuardar.onclick = async () => {
      ...
      try {
        ...
        btnGuardar.textContent = mazoEnEdicionId ? 'Actualizando...' : 'Guardando...';

        let resultado;
        if (mazoEnEdicionId) {
          resultado = await actualizarMazoAPI(mazoEnEdicionId, datosMazo);
        } else {
          resultado = await guardarMazoAPI(datosMazo);
        }

        if (resultado && resultado.exito) {
          alert(`¡Mazo "${datosMazo.nombre_mazo}" ${mazoEnEdicionId ? 'actualizado' : 'guardado'} exitosamente!`);
          mazoActual = [];
          mazoEnEdicionId = null;
          if (inputNombreMazo) inputNombreMazo.value = 'Mi Nuevo Mazo';
          if (inputDescripcion) inputDescripcion.value = '';
          if (btnGuardar) btnGuardar.textContent = 'Guardar Mazo';
          actualizarVistaMazo();
...
/**
 * Carga un mazo existente en el constructor interactivo para permitir su edición.
 */
async function cargarMazoEnConstructorParaEditar(idMazo) {
  try {
    const res = await obtenerMazoPorIdAPI(idMazo);
    const mazo = (res && res.datos) ? res.datos : res;
    if (!mazo || (!mazo.id_mazo && !mazo.nombre_mazo)) {
      alert('No se pudo cargar la información del mazo para editar.');
      return;
    }
    mazoEnEdicionId = Number(idMazo);

    const inputNombreMazo = document.getElementById('nombre-mazo');
    const inputDescripcion = document.getElementById('descripcion-mazo');
    const chkPublico = document.getElementById('mazo-publico');
    const btnGuardar = document.getElementById('btn-guardar-mazo');

    if (inputNombreMazo) inputNombreMazo.value = mazo.nombre_mazo || 'Mi Mazo Editado';
    if (inputDescripcion) inputDescripcion.value = mazo.descripcion_mazo || mazo.descripcion || '';
    if (chkPublico) chkPublico.checked = (mazo.es_publico === 1 || mazo.es_publico === true);

    if (btnGuardar) {
      btnGuardar.textContent = '💾 Guardar Cambios';
    }

    mazoActual = (mazo.cartas || []).map(c => ({
      id_carta: c.id_carta,
    ...
    console.error('Error al cargar mazo para edición:', error);
    alert('Ocurrió un error al cargar el mazo para edición.');
  }
}
...
async function abrirModalDetalleMazo(idMazo) {
  ...
  if (gridCartas) gridCartas.innerHTML = '<p class="mensaje-mazo-vacio">Cargando cartas del mazo...</p>';
  ...
  if (res && res.cartas && Array.isArray(res.cartas)) {
    ...
    if (gridCartas) {
      ...
      res.cartas.forEach(carta => {
        ...
        item.innerHTML = `
          <div class="carta-mazo-detalle-img-wrapper">
            <img src="${imgUrl}" alt="${sanitizarHTML(carta.nombre_carta)}" loading="lazy">
    ...
    if (gridCartas) gridCartas.innerHTML = '<p class="mensaje-mazo-vacio">No se pudieron cargar las cartas de este mazo.</p>';
  }
